...
def understand_batch_with_qwen_tensors(
    ...
    """
    image_tensors: Tensor of shape (B, 3, H, W), values in [0,1] or [0,255]
    captions:      list of B strings
    Returns Qwen2-VL outputs for each image+caption in the batch.
    """
    ...
    for i in range(batch_size):
        img = image_tensors[i]
        # Qwen-VL expects either a PIL.Image, file path, URL, or raw Tensor:
        messages.append({
            "role": "user",
            "content": [
    ...
    # 5) Strip off prompt tokens, decode
    stripped = [
        out_ids[len(in_ids):]
...
def clip_finetune_fiq(train_dress_types: List[str], val_dress_types: List[str],
    ...
    training_path: Path = Path("models") / f"clip_finetuned_on_fiq_blip2_cir_align_prompt_{datetime.now().strftime('%Y-%m-%d_%H_%M_%S')}"
    ...
    for epoch in range(num_epochs):
        ...
        for idx, (reference_images, target_images, captions) in enumerate(train_bar):
            ...
            img_understanding = understand_batch_with_qwen_tensors(reference_images, captions)
            print(img_understanding)
